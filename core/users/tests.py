...
def get_auth_headers(self, username: str, password: str):
    token = self.client.post(f"/users/auth/", {"username": username, "password": password}, format="json").json()["token"]
...
class UserAuthTests(APITestCase):
    ...
    def test_get_token_and_delete(self):
        # get
        response = self.client.post("/users/auth/", {"username": "u", "password": "1215"}, format="json")
        data = response.json()
        self.assertIn("token", data)

        # delete
        token = data["token"]
        response = self.client.delete("/users/auth/", HTTP_AUTHORIZATION=f"Token {token}")
        self.assertEqual(response.json(), {"success": 1})
...
class ChangeUserPasswordTests(APITestCase):
    def setUp(self):
        self.get_auth_headers = lambda *args, **kwargs: get_auth_headers(self, *args, **kwargs)
    ...
    def test_change_wrong_old(self):
        ...
        response = self.client.patch("/users/change_password/", send_data, **self.headers, format="json")
    ...
    def test_change_password(self):
        ...
        response = self.client.delete(f"/users/{self.u.pk}/", **self.headers, format="json")
        self.assertEqual(response.status_code, 401)
...
class UserDetailsTests(APITestCase):
    ...
    def test_get_user_info(self):
        ...
        self.assertEqual(set(data), {"id", "username", "info", "is_stuff", "registration_date"})
    ...
    def test_edit_and_delete_user_no_auth(self):
        response = self.client.patch(f"/users/{self.u.pk}/", {"username": "uu"}, format="json")
    ...
    def test_edit_and_delete_as_another_user(self):
        ...
        response = self.client.patch(f"/users/{self.u.pk}/", {"username": "uu"}, format="json", **headers)
    ...
    def test_edit_and_delete_admin_as_another_admin(self):
        ...
        response = self.client.patch(f"/users/{self.admin.pk}/", {"username": "uu"}, format="json", **headers)
        self.assertEqual(response.status_code, 403, f"{response.json()=}")
        response = self.client.delete(f"/users/{self.admin.pk}/", format="json", **headers)
        self.assertEqual(response.status_code, 403, f"{response.json()=}")
    ...
    def edit_and_delete_user_as_someone_who_have_permission(self, headers):
        ...
        response = self.client.patch(f"/users/{self.u.pk}/", {"info": "test info"}, format="json", **headers)
        self.assertEqual(response.status_code, 200, f"response.json()")
        self.u.refresh_from_db()
        self.assertEqual(self.u.info, "test info")

        response = self.client.patch(f"/users/{self.u.pk}/", {"username": "ua", "password": "aa"}, format="json", **headers)
        self.assertEqual(response.status_code, 400)

        response = self.client.patch(f"/users/{self.u.pk}/", {"is_stuff": True}, format="json", **headers)
        self.u.refresh_from_db()
        ...
        try:
            ...
            self.assertEqual(0, 1, f"user is not deleted, but exception was raised {e=}")
